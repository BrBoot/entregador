...
// Do some validation, check to make sure the name, email and message are valid.
   ...
   # redirectWithError("Por favor complete o CAPTCHA.");
#}
...
#$resp = $recaptcha->verify($_POST['g-recaptcha-response'], $_REQUEST['REMOTE_ADDR']);
   ...
   # redirectWithError("Por favor repita o CAPTCHA: ".$errorMessage);
#}

if (empty($_POST['name'])) {
    redirectWithError("Por favor escreva seu nome.");
}

if (empty($_POST['email'])) {
    redirectWithError("Por favor escreva seu e-mail.");
}

if (empty($_POST['escreva'])) {
    redirectWithError("Por favor escreva sua mensagem.");
}

if (!filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)) {
    redirectWithError("Por favor entre com um e-mail vádido.");
}

if (strlen($_POST['escreva']) < 10) {
    redirectWithError("Por favor escreva no minímo 10 caracteres.");
}
...
try {
    ...
    // Content
    $mail->Subject = "[Form Entregador] ".$_POST['name'];
    $mail->Body    = <<<EOT
Name: {$_POST['name']}
Email: {$_POST['email']}
...
EOT;
    ...
    redirectWithError("Um erro ocorreu ao tentar mandar sua mensagem: ".$mail->ErrorInfo);
}
